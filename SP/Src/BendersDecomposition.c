...
void Cut_Stabilization(CPXENVptr env, CPXLPptr lp, double lambda) {
	...
	do {		
		...
		//更新分离点
		for (int i = 0; i < N; i++) {
			Separation_Point[i] = lambda * Stabilizing_Point[i] + (1 - lambda) * X[i];
		}
...
double GBD_Separator(double* Z) {
	...
	double voptj = 0, gramma = 0, lambda = 0;
	double rhs = 1;
	if (P == 2.0) {//针对x^2的算例 无需规约
		//求解归约后的问题		
		for (int i = 0; i < N; i++) {
			Rlist[i] = i;
			if (Y[i] == 0) {
		...
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0) {
				...
				alpha[i] = MAX(0, lambda - quad_cost[i] * P);
				if (x[i] == Y[i])
					subgradient[i] = -alpha[i] - quad_cost[i];
				else {
					x_[i] = x[i] / Y[i];
					subgradient[i] = -alpha[i] - quad_cost[i] * x_[i] * x_[i];
				}
				voptj += 0.5 * 1 / a[i] * x[i] * x[i];
			}
		}
	}
	...
	if (P == 1.5) {//这个函数针对cx^p	
		for (int i = 0; i < N; i++) {
			...
			if (Y[i] == 0) {
				...
				a[i] = Y[i] / (quad_cost[i] * quad_cost[i]);//归约问题系数
				gramma += a[i];
			}
		}
		lambda = rhs / gramma;
		...
		for (int i = 0; i < N; i++) {
			if (Rlist[i] < N) {
				lambda = quad_cost[i] * P * sqrt(x[i] / Y[i]);
				flag = 1;
				break;
			}
		}
		...
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0) {
				...
				alpha[i] = MAX(0, lambda - quad_cost[i] * P);
				if (x[i] == Y[i]) {
					subgradient[i] = -alpha[i] + (1 - P) * quad_cost[i];
					voptj += quad_cost[i] * Y[i];
				}
				else {
					x_[i] = x[i] / Y[i];
					subgradient[i] = -alpha[i] - quad_cost[i] * 0.5 * sqrt(x_[i]) * x_[i];
					voptj += quad_cost[i] * Y[i] * sqrt(x_[i]) * x_[i];
				}
			}
		}
	}
	...
	if (P == 2.5) {//这个函数针对cx^p	
		for (int i = 0; i < N; i++) {
			...
			if (Y[i] == 0) {
				...
				a[i] = Y[i] / pow(quad_cost[i], 1 / (P - 1));//归约问题系数
				gramma += a[i];
			}
		}
		lambda = rhs / gramma;
		...
		for (int i = 0; i < N; i++) {
			if (Rlist[i] < N) {
				lambda = quad_cost[i] * P * sqrt(x[i] / Y[i]) * (x[i] / Y[i]);
				flag = 1;
				break;
			}
		}
		...
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0) {
				...
				if (x[i] == Y[i]) {
					...
					x_[i] = x[i] / Y[i];
					subgradient[i] = -alpha[i] - quad_cost[i] * 1.5 * sqrt(x_[i]) * x_[i] * x_[i];
					voptj += quad_cost[i] * Y[i] * sqrt(x_[i]) * x_[i] * x_[i];
				}
			}
		}
	}
	...
	if (P == 3.0) {//这个函数针对cx^p	
		for (int i = 0; i < N; i++) {
			...
			if (Y[i] == 0) {
				...
				a[i] = Y[i] / sqrt(quad_cost[i]);//归约问题系数
				gramma += a[i];
			}
		}
		lambda = rhs / gramma;
		...
		for (int i = 0; i < N; i++) {
			if (Rlist[i] < N) {
				lambda = quad_cost[i] * P * (x[i] / Y[i]) * (x[i] / Y[i]);
				flag = 1;
				break;
			}
		}
		...
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0) {
				...
				if (x[i] == Y[i]) {
					...
					x_[i] = x[i] / Y[i];
					subgradient[i] = -alpha[i] - quad_cost[i] * 2 * x_[i] * x_[i] * x_[i];
					voptj += quad_cost[i] * Y[i] * x_[i] * x_[i] * x_[i];
				}
			}
		}
	}

	if (P == 0) {//这个函数针对归约后的二次优化问题 P = 0 Kleinrock average delay function
		...
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0)
				...
				a[i] = sqrt(quad_cost[i]) * Y[i];//获取归约后的二次资源分配问题的参数a[]和b[]
				b[i] = -1 / sqrt(quad_cost[i]);//获取归约后的二次资源分配问题的参数a[]和b[]
		...
		for (int i = 0; i < N; i++) {
			if (S[i] != -1) {
				flag = 0;
				x_[i] = x[i] / Y[i];
				lambda = quad_cost[i] / ((1 - x_[i]) * (1 - x_[i]));//对原问题的透视函数进行修改
				break;
			}
		}
		if (flag == 1)
		...
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0) {
				//alpha[i] = lambda;
				alpha[i] = MAX(0, lambda - quad_cost[i]);//修改bug;
				subgradient[i] = -alpha[i];
			}
			else {
				alpha[i] = 0;//对原问题的透视函数进行修改			
				x_[i] = x[i] / Y[i];
				subgradient[i] = -alpha[i] - quad_cost[i] * x_[i] * x_[i] / ((1 - x_[i]) * (1 - x_[i]));//对原问题的透视函数进行修改
				voptj += quad_cost[i] * x[i] / (1 - x_[i]);//对原问题的透视函数进行修改
			}
		}
	}


	if (P == -1) {//这个函数针对归约后的二次优化问题 c e^(-x)
		//求解归约后的问题
		memset(S, -1, sizeof(int)* N);//全部初始化为-1
		double  temp = 0;
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0)
				...
				a[i] = Y[i];
				b[i] = -log(quad_cost[i]);//获取归约后的二次资源分配问题的参数a[]和b[]
		...
		for (int i = 0; i < N; i++) {
			if (S[i] != -1) {
				flag = 0;
				x_[i] = x[i] / Y[i];
				lambda = -quad_cost[i] * exp(-x_[i]);//对原问题的透视函数进行修改
				break;
			}
		}
		if (flag == 1)
		...
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0) {
				...
				alpha[i] = MAX(0, quad_cost[i] * exp(-1) + lambda);//对原问题的透视函数进行修改
				if (x[i] == Y[i]) {
					subgradient[i] = -alpha[i] + quad_cost[i] * (2 * exp(-1) - 1);//对原问题的透视函数进行修改
					voptj += quad_cost[i] * Y[i] * (exp(-1) - 1);//对原问题的透视函数进行修改
				}
				else {
					x_[i] = x[i] / Y[i];
					subgradient[i] = -alpha[i] + quad_cost[i] * (exp(-x_[i]) - 1 + x_[i] * exp(-x_[i]));//对原问题的透视函数进行修改
					voptj += quad_cost[i] * Y[i] * (exp(-x_[i]) - 1);//对原问题的透视函数进行修改
				}
			}
		}
	}

	if (P == -2) {//这个函数针对归约后的二次优化问题 - c log(1+x)
		//求解归约后的问题
		memset(S, -1, sizeof(int)* N);//全部初始化为-1
		double  temp = 0;
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0)
				...
				a[i] = quad_cost[i] * Y[i];
				b[i] = 1/quad_cost[i];//获取归约后的二次资源分配问题的参数a[]和b[]
		...
		for (int i = 0; i < N; i++) {
			if (S[i] != -1) {
				flag = 0;
				x_[i] = x[i] / Y[i];
				lambda = -quad_cost[i] / (1 + x_[i]);//对原问题的透视函数进行修改
				break;
			}
		}
		if (flag == 1)
		...
		for (int i = 0; i < N; i++) {
			if (Y[i] == 0) {
				...
				alpha[i] = MAX(0, quad_cost[i] / 2 + lambda);//对原问题的透视函数进行修改
				if (x[i] == Y[i]) {
					subgradient[i] = -alpha[i] - quad_cost[i] * log(2) + quad_cost[i] / 2;//对原问题的透视函数进行修改
					voptj -= quad_cost[i] * Y[i] * log(2);//对原问题的透视函数进行修改
				}
				else {
					x_[i] = x[i] / Y[i];
					subgradient[i] = -alpha[i] - quad_cost[i] * log(1 + x_[i]) + quad_cost[i] * x_[i] / (1 + x_[i]);//对原问题的透视函数进行修改
					voptj -= quad_cost[i] * Y[i] * log(1 + x_[i]);//对原问题的透视函数进行修改
				}
			}
		}
	}
